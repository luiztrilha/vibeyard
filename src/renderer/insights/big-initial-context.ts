...
export const bigInitialContext: InsightAnalyzer = {
  ...
  analyze(snapshot: InitialContextSnapshot): InsightResult[] {
    if (snapshot.usedPercentage >= THRESHOLD_PERCENTAGE) {
      ...
      const tokens = numberFormatter.format(snapshot.totalTokens);
      const contextWindowSize = numberFormatter.format(snapshot.contextWindowSize);
      return [{
        id: 'big-initial-context',
        severity: 'warning',
        ...
        action: {
          ...
          prompt: `My pre-context is using ${pct}% of the context window (${tokens} tokens out of ${contextWindowSize}). This is too high and impacts response quality and cost. Please analyze what's contributing to the pre-context size and help me reduce it. Check CLAUDE.md files, custom instructions, MCP server configurations, and any other sources of pre-loaded context. Suggest specific changes to bring the pre-context below 15% of the context window.`,
        },
      }];
    }
    return [];
